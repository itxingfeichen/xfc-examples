...
# 4、查询姓“猴”的老师的个数（不重要）
select count(t.t_id) count
from Teacher t
where t_name like '张%';
# 查询同姓"张"老师的个数
select count(distinct t.t_name) count
from Teacher t
where t_name like '张%';
...
# 查询01号同学学过的课程
        ...
        from Score
        where s_id = '01')
      and s_id != '01') as b on stu.s_id = b.s_id;
...
where s_id in (
    ...
    where s.c_id in (
      ...
      and s.s_id != '01'
    group by s.s_id
    having count(s.c_id) = (select count(distinct c_id) from Score where s_id = '01'))
group by s_id
having count(c_id) = (select count(distinct c_id) from Score where s_id = '01');
# 这个sql逻辑有问题，使用in筛选数据时是或的关系，并使并列的关系
...
from Student
where s_id in (
    select s.s_id from Score s where s.c_id = '01' and s.s_score < 60 order by s.s_score asc)
# 不严谨写法，排序有问题
# 严谨写法，使用join
...
select s.s_id, a.ave_s, s.s_score
from Score s
         inner join
         (select s_id, avg(s_score) ave_s from Score group by s_id) a on
         s.s_id = a.s_id
group by s.s_id, s.s_score
order by a.ave_s desc;
# 非常重要
select s_id,
...
# 方法一：以课程为维度
select t.t_name, c.c_name, avg(sc.s_score) score
from Score sc
...
order by score desc;
# 方法二：以老师为维度
select t.t_id, t_name, avg(sc.s_score) score
from Score sc
         inner join Course c on sc.c_id = c.c_id
         inner join Teacher t on c.t_id = t.t_id
...
select s_id, avg(s_score) ave, row_number() over(order by avg(s_score) desc)
from Score
group by s_id
order by ave desc;
...
select *
from Student
where month(s_birth) = '5';
select *
from Student
where day(s_birth) = '20';

# 32、查询平均成绩大于等于85的所有学生的学号、姓名和平均成绩（不重要）
select sc.s_id, stu.s_name, avg(sc.s_score) as ave
from Score sc
         inner join Student stu on sc.s_id = stu.s_id
group by sc.s_id
having ave >= 85;
...
select c_id, avg(s_score) ave
from Score
group by c_id
order by ave, c_id desc;
# 扩展
select sc.c_id, c.c_name, avg(sc.s_score) ave
from Score sc
         inner join Course c on sc.c_id = c.c_id
group by sc.c_id
order by ave, sc.c_id desc;
...
select s.s_name, c.c_name, s_score
from Score sc
         inner join Course c on sc.c_id = c.c_id
...
order by c_id desc;
# 扩展：显示学生姓名等信息
select s.s_name, c.c_name, sc.c_id, s_score
from Score sc
         inner join Course c on sc.c_id = c.c_id
         inner join Student s on sc.s_id = s.s_id
where sc.s_score < 60
...
select t.t_name, s.s_id, sc.s_score, s.s_name, c.c_name
from Score sc
         inner join Student s on sc.s_id = s.s_id
         inner join Course c on sc.c_id = c.c_id
         inner join Teacher t on c.t_id = t.t_id
...
from Score s1
    ...
    from Score s
        ...
        from Score
        group by s_id
        having count(s_id) > 1) a on s.s_id = a.s_id
    group by s.s_id, s.s_score) b on s1.s_id = b.s_id
group by s1.s_id
having count(s1.s_id) > 1;
...
select s_id, s_name, s_sex, datediff(now(), s_birth) / 365 birth
from Student;
